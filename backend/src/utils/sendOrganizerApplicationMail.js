...
/**
 * Send organizer application submitted email using Brevo API
...
const sendOrganizerApplicationMail = async (
  ...
  try {
    ...
    sendSmtpEmail.subject = "Organizer Application Submitted 🚀";
    
    // Convert recipients to Brevo array format
...
<style>
  ...
  .info-box { background: #f1f5f9; border-radius: 12px; padding: 18px; margin: 24px 0; }
...
<div class="wrapper">
  <div class="card">
    ...
    <div class="content">
      <h2>Hi ${user.name},</h2>
      <p>Thank you for applying to become an <strong>Organizer</strong> on <strong>InstantBook</strong>.</p>
      <div class="info-box">
        <strong>Organization Name:</strong><br/> ${organizationName}
      </div>
      <p>Our team is currently reviewing your application. Once approved, you’ll be able to:</p>
      <ul>
        <li>Create and publish events</li>
        <li>Sell free and paid tickets</li>
        <li>Track bookings and revenue</li>
      </ul>
      <p style="margin-top: 24px;">⏳ <strong>Status:</strong> Verification Pending</p>
      <p>We’ll notify you via email as soon as your organizer account is approved.</p>
    ...
    console.log("✅ Organizer application email sent via Brevo. ID:", data.body.messageId);

  } catch (error) {
    console.error("❌ Brevo Application Email Error:", error.response?.body || error.message);
    throw new ApiError(500, "Failed to send organizer application email via Brevo.");
  }
};
